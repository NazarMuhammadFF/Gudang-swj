...
export default function SubmissionsPage() {
  ...
  const loadSubmissions = useCallback(async () => {
    const submissionList = await db.submissions.orderBy('createdAt').reverse().toArray();
  ...
  const handleStatusUpdate = async (submission: Submission, status: Submission['status']) => {
  ...
  const handleDelete = async (submission: Submission) => {
    if (confirm(`Hapus pengajuan "${submission.productName}" dari ${submission.sellerName}?`)) {
  ...
  const emptyState = useMemo(() => submissions.length === 0, [submissions.length]);
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500 dark:text-neutral-400">
            Tinjau pengajuan barang bekas yang masuk sebelum dipublikasikan ke etalase.
          </p>
        </div>
                ...
                <TableRow key={submission.id}>
                  ...
                  <TableCell>
                    <div>
                      ...
                      <p className="text-xs text-neutral-500">{submission.category || 'Tanpa kategori'}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="capitalize">{submission.condition}</TableCell>
